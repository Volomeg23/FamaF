...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        ...
        //fprintf(stdout, "%u %u %u ", year + FST_YEAR, month + 1, day + 1);
        k_year = k_year - FST_YEAR;  //@param[in] year Year array position (start in 0 ends with YEARS - 1)
        k_month = k_month -1;   //  @param[in] month Month of the year (december is last month 11u), el .in va de 1 hasta 12(se resta -1)
        k_day = k_day -1;
