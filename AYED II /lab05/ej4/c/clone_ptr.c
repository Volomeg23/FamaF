...
int main(void) {
    ...
    //esto da error ya que estamos pidiento el sizeof original pero este nos va a dar el tamaño de la dirreccion de memoria de un puntero, no el contenido en si
    //por lo tanto siempre nos va dar 8 siempre
    copy = string_clone(original, strlen(original)/sizeof(char) - 1); // con este nos da el size de original completo sin usar *original
    ...
    copy[5] = 'g';
    printf("Copia   : %s\n", copy);
    printf("size copy : %li\n",strlen(copy)); // aca tambien si usamos sizeof nos da el size de un simple char
    free(copy);
