...
hanoi_t hanoi_init(unsigned int disk_count) {
    ...
    hanoi->target = stack_empty();  // PROBLEMA
    hanoi->source = stack_empty();  // PROBLEMA
...
hanoi_t hanoi_destroy(hanoi_t hanoi) {
    ...
    hanoi->aux = stack_destroy(hanoi->aux);  // PROBLEMA
    hanoi->target = stack_destroy(hanoi->target);  // PROBLEMA
    free(hanoi);
    return NULL;
}
