...
static bool invrep(graph_t g) {
    ...
    if (g == NULL){
        ...
        if (g->costs == NULL){
            ...
            for (unsigned int i=0u; i < g->max_vertexs; i++){
                if (g->costs[i] == NULL){
                    b = false;
                } 
            }
...
graph_t graph_empty(unsigned int max_vertexs)
{
    ...
    graph->costs = calloc(max_vertexs, sizeof(cost_t*)); // Creo una estructura o  columna

    for (unsigned int k=0; k < max_vertexs; k++){
        graph->costs[k] = calloc(max_vertexs, sizeof(cost_t)); // Creo una fila
    }
...
graph_t graph_destroy(graph_t graph)
{
    ...
    for (int j = graph->max_vertexs-1; j >= 0; j--){
        free(graph->costs[j]);
        graph->costs[j] = NULL;
    }
